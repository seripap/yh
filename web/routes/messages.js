...
module.exports = function routing(app, api, renderGenerator) {
  ...
  app.get('/messages/inbox', checkAuth, ping, function (req, res, next) {
    api.getInbox(
      ...
      req.route.query || {},
      req.session.user,
      renderGenerator.inboxHandler(req, res, next)
    );
  });
  ...
  app.get('/messages/outbox', checkAuth, ping, function (req, res, next) {
    api.getOutbox(
      ...
      req.route.query || {},
      req.session.user,
      renderGenerator.outboxHandler(req, res, next)
    );
  });
  ...
  app.get(
    ...
    function (req, res, next) {
      ...
      renderGenerator.messageSendHandler(
        req,
        res,
        next
      )(null, {
  ...
  app.get('/message/:messageid', checkAuth, ping, function (req, res, next) {
    api.getMessage(
      ...
      req.route.params || {},
      req.session.user,
      renderGenerator.messageHandler(req, res, next)
    );
  });
  ...
  app.get(
    ...
    function (req, res, next) {
      api.getMessage(
        ...
        req.route.params || {},
        req.session.user,
        renderGenerator.messageSendHandler(req, res, next)
      );
    }
  );
