...
module.exports = {
    ...
    var loadtest = nl.run({
      ...
      requestGenerator: function (client) {
        var request = client.request(
          'GET',
          '/stresstarget?_=' + Math.floor(Math.random() * 100000000)
        );
        request.end();
        return request;
      },
  ...
  newcomment: function (req, res, next) {
    api.postComment(
      ...
      function (err, comment) {
        res.redirect(
          '/thread/' + encodeURIComponent(testthread.urlname) + '#bottom'
        );
      }
    );
  },
  randomcomment: function (req, res, next) {
    ...
    api.getRandomThread(res, {}, req.session.user, function (err, json) {
      ...
      api.postComment(
        ...
        function (err, comment) {
          res.redirect(
            '/thread/' + encodeURIComponent(thread.urlname) + '#bottom'
          );
        }
      );
    });
  ...
  getindex: function (req, res, next) {
    api.getThreads(
      ...
      req.route.params || {},
      req.session.user,
      renderGenerator.threadsListingHandler(req, res, next)
    );
  },
  getthread: function (req, res, next) {
    api.getThread(
      res,
      { threadUrlName: testthread._id },
      req.session.user,
      function (err, thread) {
        res.render('thread', {
          title: thread.name,
