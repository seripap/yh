...
module.exports = {
        ...
        res.render(
          ...
          _.extend(templateData, {
            ...
            threads: _(json.threads).map(function (thread) {
              ...
              thread.numpages = (
                threadpages[threadpages.length - 1] || { num: 1 }
              ).num;
              ...
              thread.removed =
                user.hide_enemy_posts &&
                _userIgnores.indexOf(thread.postedby) > -1;
  ...
  errorPageHandler: function (req, res, renderdata, next) {
    ...
    renderUtils.getUserTemplateData(user, function (templateData) {
      ...
      res.render(
        'error',
        _.extend(templateData, gameData, {
          title: title,
          titleauthor: titleauthor,
  ...
  threadDetailHandler: function (req, res, next) {
    ...
    return function (err, json) {
      ...
      var authedUserCanPost =
        thread.postedby_ignores.indexOf(user.username) === -1;
        ...
        res.render(
          ...
          _.extend(templateData, {
            ...
            comments: _(thread.comments).map(function (comment, index) {
              var dayslater = lastcomment
                  ? moment(comment.created).diff(
                      moment(lastcomment.created),
                      'days'
                    )
                  : 0,
                newcomment = _.extend(
                  {
                    ...
                    removed:
                      user.hide_enemy_posts &&
                      _userIgnores.indexOf(comment.postedby) > -1,
                    toggleSourceLabel:
                      (index === 0 && user.username === comment.postedby) ||
                      (comment.postedby === user.username &&
                    ...
                    canpoint:
                      templateData.canpoint &&
                      user.username !== comment.postedby,
                    showthreadcontrols:
                      index === 0 &&
                      activepage === 1 &&
                      user.username === thread.postedby,
  ...
  userDetailHandler: function (req, res, next) {
    ...
    return function (err, selecteduser) {
      ...
      var _userBuddies = _(user.buddies || []),
        ...
        canpoint =
          new Date().getTime() - new Date(lastpointusage).getTime() > pointtime,
        realname =
          selecteduser.realname && selecteduser.realname.length
            ? selecteduser.realname
            : false,
        location =
          selecteduser.location && selecteduser.location.length
            ? selecteduser.location
            : false;

      WhosOnline.activeUsers([selecteduser.username], function (onlineuser) {
        if (!canpoint) {
          nextpointtime = moment(lastpointusage)
            .add(pointtime / 1000, 'seconds')
            .fromNow();
        }
        ...
        renderUtils.getUserTemplateData(user, function (templateData) {
          res.render(
            ...
            _.extend(templateData, {
              ...
              lastlogin:
                lastlogin.format('MMMM D YYYY') +
                ' at ' +
                lastlogin.format('h:mm a'), //September 16th 2013 at 4:47 pm
              ...
              comments: _(selecteduser.comments || []).reduce(function (
                memo,
                comment
              ) {
  ...
  newThreadHandler: function (req, res, next) {
    ...
    return function (err, postedData) {
      ...
      renderUtils.getUserTemplateData(user, function (templateData) {
        res.render(
          'post',
          _.extend(templateData, postedData, {
  ...
  pendingRegistrationsHandler: function (req, res, next) {
    ...
    return function (err, data) {
      pendingApplicants.update(function () {
        renderUtils.getUserTemplateData(user, function (templateData) {
          templateData.pendingUsers = pendingApplicants
            .getPendingApplicants()
            .map(function (applicant) {
              applicant.pointstext =
                applicant.points +
                ' point' +
                (applicant.points === 1 ? '' : 's');
              return applicant;
            });
