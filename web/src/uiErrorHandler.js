...
routes = {
    ...
    ValidatorError: {
      'Old password failed validation': function (err, req, res, next) {
        preferencesError(
          ...
          res,
          next,
          'Your original password must be at least 6 characters.'
        );
      },
  ...
  register: {
    ...
    Error: {
      'Error: a pending user already exists for that ip': function (
        err,
        req,
        res,
        next
        ...
        registerError(
          ...
          res,
          next,
          'There is already a pending user for your IP'
        );
      },
    },
  },
    ...
    ValidatorError: {
      ...
      'Username failed validation': function (err, req, res, next) {
        passwordResetError(
          ...
          res,
          next,
          'There is an error with your username'
        );
      },
      'New password failed validation': function (err, req, res, next) {
        passwordResetError(
...
function getErrorMessage(err) {
  ...
  if (isMongoDuplicateError(err)) {
    mongoDupKeynameMatches = err.message.match(
      /duplicate key error index:.*?\..*?\.\$(.*?)_/
    );
...
module.exports = {
  handleError: function (err, req, res, next, errorSourceKey) {
    var errorType, errorMsg, sourceRoute, errorTypesRoute, errorHandler;
