...
function format_special(element) {
  ...
  $(element).each(function () {
    ...
    $(this)
      ...
      .each(function () {
        ...
        $(this)
          .prepend('<div class="tqname">' + user + ' said:</div>')
          .addClass('tquote');
      });
  });
  ...
  $('spoiler').each(function () {
    var warning_msg = 'Warning! May contain spoilers. Click to reveal.',
      ...
      $disclaimer = $('<div class="spoiler-disclaimer"></div>').text(
        warning_msg
      ),
...
if (document.getElementById('thread-content-input')) {
  document.getElementById('thread-content-input').addEventListener(
    'drop',
    function (e) {
      e.preventDefault();
          ...
          $.ajax({
            ...
            error: function (response) {
              if (response.status === 413) {
                $this.parent().find('.error,.loading').remove();
                $this.after(
                  '<p class="error">Image too large, maximum file size for pasted images is 2MB</p>'
                );
              }
            },
          });
...
thread = {
  status_text: {
    nsfw: ['Unmark Naughty', 'Mark Naughty'],
        ...
        thread.comments[comment_id] = {
          ...
          data: {
            content: data.content
              ...
              .replace(/<br \/>/g, '\n'),
            owner:
              (data.postedby === currentUser &&
                $('#comment-' + comment_id + ' #thread-control').length ===
                  1) ||
              moment(data.created).diff(new Date()) > -3600000,
          },
          author: author,
        };
        callback();
  ...
  quote: function (comment_id) {
    if (thread.comments[comment_id] !== undefined) {
      if (
        (selected.comment_id && selected.comment_id != comment_id) ||
        !selected.html
      ) {
      ...
      html =
        '<blockquote title="' +
        $.trim(thread.comments[comment_id].author) +
        '">' +
        content +
        '</blockquote>';
      ...
      $('#thread-content-input').scrollTop(
        $('#thread-content-input')[0].scrollHeight -
          $('#thread-content-input').height()
      );
      $('#thread-content-input').selectRange(
        $('#thread-content-input').val().length
      );
    } else {
    ...
    $.ajax({
      ...
      success: function (data) {
        ...
        if (Math.floor(data.edit_percent) > 0) {
          html +=
            '<div class="edited-percent">Edited ' +
            Math.floor(data.edit_percent) +
            '%</div>';
        }
  ...
  view_original: function (comment_id) {
    if (thread.comments[comment_id] !== undefined) {
      $('#comment-' + comment_id + ' .content').html(
        thread.comments[comment_id].rendered
      );
    }
  },
    ...
    if (thread.comments[comment_id] !== undefined) {
      ...
      if (comment.data.owner) {
        comment.container.append(
          $('<button>', { html: 'Save' }).bind('click', function () {
            thread.save(comment_id);
          })
        );
      }

      comment.container.append(
        $('<button>', { html: 'Close' }).bind('click', function () {
          thread.view_original(comment_id);
        })
      );
    } else {
...
(function ($) {
  ...
  $notifications
    .on('click', '#closenotify', function (e) {
      e.preventDefault();
      $notifications.hide();
    })
  ...
  threadEvents.addEventListener(
    'message',
    function (e) {
      postcount++;

      $('title').text(
        [postcount, ' new posts | ', originalThreadTitle].join('')
      );

      $notifications
        .html(
          '<a id="closenotify"></a><div id="notifier"><a id="notify" href="">' +
            postcount +
            ' new post' +
            (postcount === 1 ? '' : 's') +
            ' added</a></div>'
